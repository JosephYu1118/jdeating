...
new Vue({
  mounted () {
    $(window).scroll(() => {
      let offsetClassificationBox = $('.classification_box').offset().top - 100
      let offsetContact = $('#feedback_sheet').offset().top - 90
      if ($(window).scrollTop() >= offsetClassificationBox) {
      ...
      } else if ($(window).scrollTop() < offsetClassificationBox) {
      ...
      if ($(window).scrollTop() >= offsetContact) {
        $('.logo').removeClass('logo_black')
        $('.menu').removeClass('menu_black')
      }
    })
...

{
  ...
  class Item {
    constructor (el, options) {
      ...
      this.DOM.animatable = {}
      this.DOM.animatable.image = this.DOM.el.querySelector('.thumbnail_img')
      this.DOM.animatable.title = this.DOM.el.querySelector('.article_title')
      this.DOM.animatable.text = this.DOM.el.querySelector('.article_description')
      this.DOM.animatable.shadow = this.DOM.el.querySelector('.thumbnail_shadow')
      this.initEvents()
    }
    initEvents () {
      let enter = false
      ...
      this.mouseleaveFn = ev => requestAnimationFrame(() => {
        ...
        for (let key in this.DOM.animatable) {
          ...
          TweenMax.to(this.DOM.animatable[key], this.options[key].reverseAnimation !== undefined ? this.options[key].reverseAnimation.duration || 0 : 1.5, {
    ...
    tilt (ev) {
      ...
      for (let key in this.DOM.animatable) {
        ...
        let r = this.options[key] !== undefined ? this.options[key].rotation || { x: 0, y: 0, z: 0 } : { x: 0, y: 0, z: 0 }
        setRange(t)
        setRange(r)
        const transforms = {
          translation: {
  ...
  class Overlay {
    constructor () {
      ...
      this.DOM.reveal = this.DOM.el.querySelector('.overlay_transition')
      this.DOM.items = this.DOM.el.querySelectorAll('.overlay_item')
      this.DOM.close = this.DOM.el.querySelector('.cross')
    }
    show (contentItem) {
      this.contentItem = contentItem
      this.DOM.el.classList.add('overlay_opened')
      TweenMax.to(this.DOM.reveal, 0.5, {
        ease: 'Power1.easeInOut',
        x: '0%',
        onComplete: () => {
          document.body.classList.add('preview_opened')
    ...
    revealItem () {
      ...
      let itemElems = []
      itemElems.push(this.contentItem.querySelector('.thumbnail_shadow'))
      itemElems.push(this.contentItem.querySelector('.thumbnail_img'))
      itemElems.push(this.contentItem.querySelector('.article_title'))
      itemElems.push(this.contentItem.querySelector('.article_description'))
      itemElems.push(this.contentItem.querySelector('.article_box'))
    ...
    hide () {
      this.DOM.el.classList.remove('overlay_opened')
      TweenMax.to(this.DOM.reveal, 0.5, {
        ease: 'Power3.easeOut',
        x: '0%',
        onComplete: () => {
          this.DOM.close.style.opacity = 0
          document.body.classList.remove('preview_opened')
  ...
  class Grid {
    constructor (el) {
      this.DOM = { el: el }
      this.items = []
      Array.from(this.DOM.el.querySelectorAll('.grid_item')).forEach((item) => {
        const itemObj = new Item(item)
        this.items.push(itemObj)
        if (!item.classList.contains('grid_item--noclick')) {
  ...
  let allowTilt = true
  const gridEl = new Grid(document.querySelector('.grid'))
  console.log(gridEl)
  imagesLoaded(document.querySelectorAll('.thumbnail_img'), () => document.body.classList.remove('loading'))
  const gridItem = [...$("[class ^= 'grid_item']")]
  const overlayItem = [...$("[class ^= 'overlay_item']")]
  gridItem.forEach((item, index) => {
    item.addEventListener('click', function () {
      overlayItem.forEach(item => {
        $(item).css('display', 'none')
      })
      $(overlayItem[index]).css('display', 'flex')
      $('.overlay').scrollTop(0)
      setTimeout(() => {
        $('.home_page').css('opacity', 0)
      }, 1000)
    })
  })
}
